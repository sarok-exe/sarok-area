...
MouseArea {
    ...
    // property list<var> clients: Niri.getActiveWorkspaceWindows()

    property list<var> clients: []

    function checkClientRects(x: real, y: real): void {
        for (const client of clients) {
            if (!client || typeof client.screenX === 'undefined')
                continue;
            const cx = client.screenX;
            const cy = client.screenY;
            const cw = client.screenW;
            const ch = client.screenH;
    ...
    Component.onCompleted: {
        ...
        if (c) {
            ...
            sx = c.screenX;
            sy = c.screenY;
            ex = c.screenX + c.screenW;
            ey = c.screenY + c.screenH;
        } else {
